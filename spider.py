...
class spider:
    ...
    def __init__(self):
        ...
        self.pages = 2  # 希望爬的总数（主要是想爬到一定阶段可以停下来，不用暴力关闭程序）
        self.num = 0
    ...
    def add_web(self, url, url_type = 0):
        ...
        if(url_type == 0):
            ...
            self.doc_url[self.str_to_id[url]] = {"text" : ''}
    ...
    def getAllUrl(self, head_url):
        ...
        # 在字典中加入标题的相关信息
        self.text_url[self.str_to_id[head_url]]["title"] = title
        self.text_url[self.str_to_id[head_url]]["text"] += title
    ...
    def getSomeUrl(self, url):
        ...
        browser.save_screenshot(os.path.join(os.path.dirname(__file__), "pictures", str(self.str_to_id[url]) + '.png'))
        # 关闭phantomjs浏览器，不要忽略了这一步，否则你会在任务浏览器中发现许多任务
        browser.quit()
        #self.driver.save_screenshot(os.path.join(os.path.dirname(__file__), "pictures", str(self.str_to_id[url]) + '.png'))
        ...
        for father_tag in ['a', 'area']:
            ...
            for tag in tags:
                try:
                    ...
                    # print(anchor_text)
                    # 若这个url已经在数据中，则直接将锚文本加入其文本中
                    if link in self.str_to_id.keys():
                        id = self.str_to_id[link]
                        if id in self.text_url.keys():
                            self.text_url[id]["text"] += anchor_text + '\n'
                        else:
                            self.doc_url[id]["text"] += anchor_text + '\n'
                        continue
                    # 若这个url已经访问过且不在数据中，说明是我们不需要的，直接跳过
                    elif link in self.skip_urls:
                        continue
                    ...
                    if response.headers['Content-Type'] == 'text/html':
                        ...
                        if type(title) != type(None):
                            if "nankai" not in url and "nankai" not in title and "南开" not in title and "nankai" not in anchor_text and "南开" not in anchor_text:
                                continue
                        elif "nankai" not in url and "nankai" not in anchor_text and "南开" not in anchor_text:
                            continue
                        ...
                        self.text_url[self.str_to_id[link]]["title"] = str(title)  # 将标题数据放入字典中
                        self.text_url[self.str_to_id[link]]["text"] += str(title) + '\n' + str(anchor_text) + '\n'
                        if self.str_to_id[link] not in self.text_url[self.str_to_id[url]]["related url"].keys():
                            self.text_url[self.str_to_id[url]]["related url"][self.str_to_id[link]] = str(anchor_text) + '\n'
                        else:
                            self.text_url[self.str_to_id[url]]["related url"][self.str_to_id[link]] += str(anchor_text) + '\n'
                    # 当前网页是文件网页
                    else:
                        self.add_web(link, 1)
                        self.doc_url[self.str_to_id[link]]["text"] += str(anchor_text) + '\n'
                        if self.str_to_id[link] not in self.text_url[self.str_to_id[url]]["related url"].keys():
                            self.text_url[self.str_to_id[url]]["related url"][self.str_to_id[link]] = str(anchor_text) + '\n'
                        else:
                            self.text_url[self.str_to_id[url]]["related url"][self.str_to_id[link]] += str(anchor_text) + '\n'
    ...
    def save_url(self):
        ...
        with open(os.path.join(os.path.dirname(__file__), "url.txt"), "wb") as file:
            pkl.dump([self.num, self.str_to_id, self.id_to_str, self.skip_urls, self.text_url, self.doc_url, queue_urls], file)
...
if __name__ == "__main__":
    ...
    # 从南开官网开始爬虫（第一次开始爬时使用）
    this_spider.getAllUrl("https://www.nankai.edu.cn")
